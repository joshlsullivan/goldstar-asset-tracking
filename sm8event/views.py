...
def process_task(customer_resource_url, token):
    ...
    if task['task_complete'] == "0":
        ...
        job_task_time_difference = date_aware_task_completed_time - job_created_time
...
def asset_tracking_event(request):
    ...
    if event == 'Webhook_Subscription':
        ...
        response_time = Task.objects.aggregate(Avg(float(int('job_task_time_difference'))), Max(float(int('job_task_time_difference'))), Min(float(int('job_task_time_difference'))))
