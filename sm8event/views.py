...
def asset_tracking_event(request):
    ...
    if event == 'Webhook_Subscription':
        ...
        response_time = Task.objects.aggregate(Avg(int(float('job_task_time_difference'))), Max(int(float('job_task_time_difference'))), Min(int(float('job_task_time_difference'))))
